...
## Starting and Resuming Cycles with Prepared Prompts

Для запуска нового цикла или возобновления прерванной работы в корне проекта подготовлены специализированные промпты. Они уже содержат актуальный контекст, ссылки на все необходимые артефакты и оптимизированы под **Blackbox + MiniMax M2.7**.

### Когда какой промпт использовать

| Ситуация                                      | Рекомендуемый промпт                              | Примечание |
|-----------------------------------------------|---------------------------------------------------|----------|
| **Старт нового цикла / новой фазы**           | `orchestrator_start_prompt_template.md` или готовый `orchestrator_start_prompt_cycleX_phaseY.md` | Используй шаблон, если ещё нет готового промпта под фазу. |
| **Возобновление прерванной работы**           | **`orchestrator_resume_prompt.md`** (приоритетно) | **Обязательно используй этот промпт**, если существует `last_agent_completion.json`. |
| **Первое включение после долгого перерыва**   | `orchestrator_resume_prompt.md`                   | Даже если файла `last_agent_completion.json` нет — этот промпт лучше структурирует восстановление контекста. |
| **Обычный запуск через Agent-Init.ps1**       | Промпт, сгенерированный скриптом                  | Подходит для простых стартов. Для сложных фаз лучше использовать подготовленные промпты выше. |

### Особенности `orchestrator_resume_prompt.md`

- Содержит специальный блок для анализа `last_agent_completion.json`.
- Если файл существует — агент **обязан** сначала его прочитать и учесть результат предыдущей работы.
- Рекомендуется как основной инструмент для продолжения работы после остановки, сбоя сессии или долгого перерыва.

### Рекомендуемый порядок действий

1. Запусти `Agent-Init.ps1` (для подготовки окружения и UTF-8 defaults).
2. Выбери нужный промпт по таблице выше.
3. При необходимости замени плейсхолдеры на актуальные значения.
4. Отправь промпт в Blackbox как первое сообщение нового/продолжающегося цикла.
