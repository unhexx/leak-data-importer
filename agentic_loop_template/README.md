...
A complete, self-contained template for running a closed-loop, self-improving multi-role agentic development cycle optimized for **MiniMax M2.7** (Blackbox and similar environments).

В шаблон добавлена продвинутая система workspace-scoped структурированной памяти, которая позволяет циклу накапливать институциональную память проекта и значительно снижать повторение ошибок.
...
├── README.md
├── SYSTEM_PROMPT.md              # Main system prompt (оптимизирован под M2.7)
├── AGENT_ROLES.md                # Роли с поддержкой структурированной памяти
├── HANDOFF_SCHEMA.md
├── TOOLS_REGISTRY.md
...
├── Agent-Init.ps1                # Robust setup for Blackbox + MiniMax M2.7 (auto-heals broken .venv)
└── Agent-Init.md                 # Detailed guide for Blackbox / MiniMax M2.7
```
