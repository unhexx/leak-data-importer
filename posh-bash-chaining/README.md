...
Обычные AI-агенты в VSCode часто запускают команды в **новых процессах** PowerShell, а не в открытом терминале. Из-за этого обычное подключение через профиль может не срабатывать.

`Install.ps1` решает эту проблему — он добавляет загрузку в самое начало профиля через `Profile-Bootstrap.ps1`. Благодаря этому инструмент подключается даже в неинтерактивных сессиях.
